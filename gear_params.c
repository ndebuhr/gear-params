...
{  
  ...
  struct gear_mod spur_mod, rack_mod, metric_mod;
  spur_mod.index='S';
  spur_mod.func=&spur_gears;
  rack_mod.index='R';
  rack_mod.func=&rack_pinion;
  metric_mod.index='U';
  metric_mod.func=&metric_US;
  struct gear_mod modules[MODULES]={spur_mod, rack_mod, metric_mod};
  char * input_file;
...
char choose_mod(char predet)
{
  ...
  char char_dump;
  bool valid_input;
  char mod_options[MODULES][MODULE_PARTS][MODULE_PART_SIZE]=
  ...
  if (predet=='\0')
    {
      ...
      for (i=0; i<MODULES; i++)
	printf("%4s  | %s\n",mod_options[i][0],mod_options[i][1]);
  ...
  do {
    valid_input=module_exists(user_choice,mod_options,MODULES);
...
bool module_exists(char user_choice, char mod_options[MODULES][MODULE_PARTS][MODULE_PART_SIZE], const int num_modules)
{
  int i;
  for (i=0; i<num_modules; i++)
    {
      if (user_choice == mod_options[i][0][0]) //limited to single digit module indexes
	return true;
    }
  return false;
}
